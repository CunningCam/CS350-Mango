...
public class ReportDao extends BaseDao {
    ...
    private static final String REPORT_INSTANCE_POINTS_INSERT = "insert into reportInstancePoints " //
            + "(reportInstanceId, dataSourceName, pointName, dataType, startValue, textRenderer, colour, consolidatedChart, scatter, title, xlabel, ylabel, yref) "
            + "values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    public static class PointInfo {
        ...
        public PointInfo(DataPointVO point, String colour, boolean consolidatedChart, boolean scatter,
            String title, String xlabel, String ylabel, double yref) {
            this.point = point;
            this.colour = colour;
        ...
        for (PointInfo pointInfo : points) {
            ...
            int reportPointId = doInsert(
                    ...
                    new Object[] {
                        ...
                        pointInfo.getColour(), boolToChar(pointInfo.isConsolidatedChart()),
                        boolToChar(pointInfo.isScatter()), pointInfo.getTitle(),
                        pointInfo.getXlabel(), pointInfo.getYlabel(),
                        pointInfo.getYref() },
                        new int[] {
    ...
    private static final String REPORT_INSTANCE_POINT_SELECT = "select id, dataSourceName, pointName, dataType, " // 
            + "startValue, textRenderer, colour, consolidatedChart, scatter, title, xlabel, ylabel, yref from reportInstancePoints ";
        ...
        List<ReportPointInfo> pointInfos = query(REPORT_INSTANCE_POINT_SELECT + "where reportInstanceId=?",
                new Object[] { instanceId }, new GenericRowMapper<ReportPointInfo>() {
                    public ReportPointInfo mapRow(ResultSet rs, int rowNum) throws SQLException {
                        ...
                        rp.setScatter(charToBool(rs.getString(9)));
